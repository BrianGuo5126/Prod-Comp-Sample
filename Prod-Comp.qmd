...
ggplot(
  ...
  labs(
    title = "Production & Comprehension Gap\nthrough Age",
    x = "Age",
    y = "Number Correct",
    colour = "Task"
  ) +
...
ggplot(
  ...
  labs(
    title = "Production & Comprehension Differences\nConsidering Sex",
    x = "Age",
    y = "Number Correct",
    colour = "Task",
    shape = "Gender"
...
ggplot(df_summary, aes(x = response.age, y = mean_rt, colour = factor(difficulty))) +
  ...
  labs(
    title = "Reaction Time vs Age\nfor Comprehension Tasks by Difficulty",
    x = "Age",
...
# Add age and gender from df_demo
df_accuracy_rt <- df_accuracy_rt |>
  left_join(df_demo, by = "ppt_id")

# Plot with multi-color gradient for age and single smooth curve
ggplot(df_accuracy_rt, aes(x = accuracy, y = mean_rt, colour = response.age, 
  ...
  scale_colour_gradientn(
    colours = rainbow(6),
    breaks = sort(unique(df_accuracy_rt$response.age))
  ) +
  labs(
